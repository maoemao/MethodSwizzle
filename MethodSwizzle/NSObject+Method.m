...
+(BOOL)test:(NSString*)name theClass:(Class)theClass value1:(SEL*)value1 value2:(IMP*)value2 value3:(Method*)value3
{
    @try{
        ...
        for(int i =0; i < methodCount; i++)
        {
            ...
            const char* methodName = (const char*)(sel_getName(methodStruct->name));
...
+(BOOL)test2:(NSString *)name theClass:(Class)theClass value1:(SEL *)value1 value2:(IMP *)value2 value3:(Method *)value3
{
    ...
    return [NSObject test:name theClass:theClass value1:value1 value2:value2 value3:value3];
}
...
+(void)listAll:(Class)theClass
{
    ...
    NSLog(@"vars for %s", class_getName(theClass));
    ...
    NSLog(@"properties for %s", class_getName(theClass));
    ...
    NSLog(@"methods for %s", class_getName(theClass));
    for(int i=0; i < methodsCount; i++)
    {
        ...
        NSLog(@"%s   %s", methodName, methodReturnType);
